...
var unid, uuid, ugid, utid, upid, unew, aqcName;    //获取随机nid, 获取userid, 随机的评论, 随机获取的parentId, 随机获取的pid, 随机获取newsCode 
...
function rand() {
  let key = ["苹果", "华为", "百度", "一个", "暴风", "王者", "联想", "小米", "可口可乐", "百事可乐", "大润发", "沃尔玛", "上汽五菱", "广汽本田"];
    ...
    if (typeof $request !== "undefined") {
        ...
        for (let i = 0; i < aqcckArr.length; i++) {
            if (aqcckArr[i]) {
                ...
                await aqccxrw(`${$.index}`);
                await $.wait(3000);
                if(taskList.length > 0 || claimList.length > 0){
                  await zxrw();
...
//发表观点函数(post)
function fbgd(gpi, npi, upi, timeout = 0) {
  return new Promise((resolve) => {
      let url = {
          url: `https://aiqicha.baidu.com/smart/addReplyAjax`,
          headers: aqchd,
          body: `content=${gpi}&nid=${npi}&replyUserId=${upi}`,
      }
...
//获取随机nid函数
async function sjnid(timeout = 0) {
  return new Promise((resolve) => {
      let url = {
          url: `https://aiqicha.baidu.com/smart/getHomeQuestionListAjax?page=1&size=10&type=hot`,
          headers: aqchd,
      }
      $.get(url, async (err, resp, data) => {
          try {
              ...
              if (qqfh.status == 0) {
                ...
                var n = Math.floor(Math.random() * nidArr.length);
                unid = nidArr[n]
                return unid; 
              }
          } catch (e) {
...
//获取userid
async function sjuid(timeout = 0) {
  await sjnid()
  return new Promise((resolve) => {
      let url = {
          url: `https://aiqicha.baidu.com/smart/questionDetailAjax?nid=${unid}`,
          headers: aqchd,
      }
      $.get(url, async (err, resp, data) => {
          try {
...
//获取随机观点函数
async function sjgid(timeout = 0) {
  await sjnid()
  return new Promise((resolve) => {
      let url = {
          url: `https://aiqicha.baidu.com/smart/questionCommentListAjax?nid=${unid}&page=1&size=20`,
          headers: aqchd,
      }
      $.get(url, async (err, resp, data) => {
          try {
              ...
              if (qqfh.status == 0) {
                ...
                var g = Math.floor(Math.random() * gidArr.length);
                ugid = gidArr[g]
                return ugid; 
              }
          } catch (e) {
...
//获取点赞观点的parentId
async function sjtid(timeout = 0) {
  await sjnid()
  return new Promise((resolve) => {
      let url = {
          url: `https://aiqicha.baidu.com/smart/questionCommentListAjax?nid=${unid}&page=1&size=20`,
      ...
      $.get(url, async (err, resp, data) => {
          try {
              ...
              if (qqfh.status == 0) {
                ...
                var g = Math.floor(Math.random() * pidArr.length);
                utid = pidArr[g]
                return utid; 
              }
          } catch (e) {
...
//获取随机pid函数
async function sjpid(timeout = 0) {
  return new Promise((resolve) => {
      let url = {
          url: `https://aiqicha.baidu.com/smart/getHomeQuestionListAjax?page=1&size=10&type=hot`,
          headers: aqchd,
      ...
      $.get(url, async (err, resp, data) => {
          try {
              ...
              if (qqfh.status == 0) {
                ...
                var p = Math.floor(Math.random() * pidArr.length);
                upid = pidArr[p]
                return upid; 
              }
          } catch (e) {
...
//获取随机新闻newsCode
async function sjnew(timeout = 0) {
  return new Promise((resolve) => {
      let url = {
          url: `https://aiqicha.baidu.com/yuqing/latestLyricalAjax?page=1&type=list`,
          headers: aqchd,
      }
      $.get(url, async (err, resp, data) => {
          try {
...
async function aqccxrw(api,timeout = 0) {
  alltaskList = []; ytaskList = []; taskList = []; claimList = [];
  return new Promise((resolve) => {
      let url = {
...
//做任务模块
async function zxrw(){
  await sjnid(), await sjuid(), await sjgid(), await sjtid(), await sjpid(), await sjnew()
  if(taskList.length > 0){
    for(j=0; j<taskList.length; j++){
      ...
      if(dqrw.title == "CX10002"){
        ...
        await $.wait(3000)
      }else if(dqrw.title == "CX11006" && unew){
        console.log("浏览新闻");
        await aqcmb(`m/getYuqingDetailAjax?yuqingId=${unew}&type=report`)
        await $.wait(3000)
        ...
        await $.wait(3000)
      }else if(dqrw.title == "CX12001" && upid){
        console.log("添加监控");
        await aqcmb(`zxcenter/addMonitorAjax?pid=${upid}`)
        await $.wait(3000)
        await aqcmb(`zxcenter/cancelMonitorAjax?pid=${upid}`)
        await $.wait(3000)
      }else if(dqrw.title == "CX12002" && upid){
        console.log("添加关注");
        await aqcmb(`my/addCollectAjax?pid=${upid}`)
        await $.wait(2000)
        ...
        await $.wait(3000)
      }else if(dqrw.title == "CX12009" && unid && utid){
        console.log("浏览互助");
        await aqcmb(`smart/questionDetailAjax?nid=${unid}`)
        await $.wait(3000)
        await aqcmb(`smart/updownAjax?nid=${unid}&parentId=${utid}&undoType=0`)
        await $.wait(3000)
      }else if(dqrw.title == "CX12010" && ugid && unid && uuid){
        console.log("发表观点");
        await fbgd(`${encodeURI(ugid)}`, `${unid}`, `${uuid}`)
        await $.wait(3000)
      }else if (dqrw.title == "CX12011" && unid && utid){
        console.log("点赞观点");
        await aqcmb(`smart/updownAjax?nid=${unid}&parentId=${utid}&undoType=0`)
        await $.wait(3000)
      }
...
async function cxad(timeout = 0) {
    ...
    return new Promise((resolve) => {
        let url = {
            url: `https://aiqicha.baidu.com/usercenter/getvipinfoAjax`,
            headers: aqchd,
        }
        $.get(url, async (err, resp, data) => {
            try {
                qqfh = JSON.parse(data)
                if (qqfh.status == 0) {
                  console.log(`账号【${aqcName}】剩余【${qqfh.data.consume}】爱豆`);
                  $.message = `账号【${aqcName}】剩余【${qqfh.data.consume}】爱豆`
                  message()
                }
            } catch (e) {
            } finally {
