// env.js 全局
const $ = new Env("闪开来电签到");
const ckName = "skld_ck";
//-------------------- 一般不动变量区域 -------------------------------------
...
class UserInfo {
    constructor(str) {
        this.index = ++userIdx;
        this.token = str; // 用户token
        this.signinStatus = false; // 是否签到过
        this.todayAward = 10; // 签证签到可得积分
        this.curAward = 0; // 本次获得积分
        this.totalAward = 0;  // 总积分
        this.adCode = '420000'; // 邮编
        this.ckStatus = true;
    }
    ...
    async signin() {
        ...
        try {
            const options = {
                //签到任务调用签到接口
                url: `https://wemp.issks.com/lottery/v1/welfare/signIn/insertRecord?award=${this.todayAward}`,
                //请求头, 所有接口通用
                headers: {
 ...
 //签到额外
    ...
    async getRecord() {
        try {
            const options = {
                //签到任务调用签到接口
                url: `https://wemp.issks.com/lottery/v1/welfare/signIn/record`,
                //请求头, 所有接口通用
                headers: {
                    'Accept-Encoding': "gzip,compress,br,deflate",
                    "Content-Type": "application/json;charset=utf-8",
            ...
            if (result?.success) {
                $.log(`✅查询记录成功`)
                this.signinStatus = result?.data?.isSign;
                $.log(`今日签到状态：${this.signinStatus ? "已签到" : "未签到"}`)
                if (!this.signinStatus) {
                    for (let record of result?.data?.signRecords) {
                        if (record?.label === '今天') {
                            this.todayAward = record?.integral;
                            break;
                        }
                    }
                }
    ...
    async cx() {
        try {
            const options = {
                url: `https://wemp.issks.com/lottery/v1/welfare/toTaskPage?adcode=${this.adCode}`,
                headers: {
                    'Accept-Encoding': "gzip,compress,br,deflate",
                    "Content-Type": "application/json;charset=utf-8",
                    "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 16_5 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.44(0x18002c2f) NetType/WIFI Language/zh_CN",
            ...
            //console.log(result)
            if (result?.success) {
                $.log(`✅查询成功 拥有积分 ${result?.data?.iflashAmount} 个!`)
                this.totalAward = result?.data?.iflashAmount;
            } else {
                $.log(`❌查询失败!${result?.msg}`)
            }
        } catch (e) {
