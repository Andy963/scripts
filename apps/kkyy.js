    ...
    async learnTime() {
        ...
        try {
            let body = {
                ...
                "Terminal": "1",
                "ApVersion": `${this.apVersion}`,
                "Method": "customer_kekeaddstudytimelog",
                "Version": `${this.version}`,
                "AppFlag": 0,
                "Sign": `${this.sign}`,
                "ApTime": `${getTimeStr()}`,
            ...
            const options = {
                ...
                headers: {
                    ...
                    'Content-Type': `application/json`,
                    'Host': `mob2015.kekenet.com`,
                    'User-Agent': `KeKeEnglish/4.9.7 (iPhone; iOS 16.5; Scale/2.00)`,
                    'Accept-Language': `en-CN;q=1, zh-Hans-CN;q=0.9`
                },
                body: JSON.stringify(body)
            };
        ...
        * */
        try {
            let body = {
                ...
                "Terminal": "1",
                "ApVersion": `${this.apVersion}`,
                "Method": "listen_video_addlistensign",
                "Version": `${this.version}`,
                "AppFlag": 0,
                "Sign": `${this.sign}`,
                "ApTime": `${getTimeStr()}`,
    ...
    async getCheckInReward(){
       ...
       * */
        try {
            ...
            let result = await httpRequest(options);
            if (result?.Code === 200) {
                $.log(`✅每日听说获取奖励成功！获得20可可豆`);
                this.signinStatus = true;
            } else {
                $.log(`❌每日听说获取奖励失败!${result?.Msg}`);
            }
        } catch (e) {
            console.log(e);
        }
...
async function main() {
    ...
    for (let user of userList) {
        if (user.ckStatus) {
            ...
            await user.cx();
            await user.signin();
            await user.learnTime();
            await user.checkIn();
            await user.getCheckInReward();
        } else {
            //将ck过期消息存入消息数组
